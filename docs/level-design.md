...
- `PlayerSpawn`: place exactly one in each playable level. Set `facing` to `up`, `down`, `left`, or `right`.
- `Door`: set `id`, `targetLevel`, `targetDoor`, `locked`, and `requiresKey` when locked.
...
- Collision drawn visually in `Tiles` but missing from `Collision`.
